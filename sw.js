...
window.openGameDetail = function (gameData, playerName = null) {
  ...
  // ランキングから開いた場合：GASの直近20能力（選択タブ期間内）を表示
  if (playerName) {
...
function renderPlayerStats(playerName, s) {
  ...
  // ★平均入れ球：GAS側が avgPins を返す想定。未対応の場合は旧 ballsPerMiss を表示（ただし意味が異なる）
  const avgPins = (s.avgPins !== undefined && s.avgPins !== null) ? s.avgPins : s.ballsPerMiss;
  document.getElementById('ps-bpm').textContent = (avgPins == null) ? '---' : `${avgPins}`;
...
window.fetchRanking = function (type = 'today') {
  ...
  fetch(fetchUrl, { cache: "no-store" })
    ...
    .then(data => {
      ...
      data.forEach((row) => {
        // GAS返却（ランキング）:
        // [0]rank [1]bestTs [2]name [3]bestScore [4]potting [5]avgPins [6]X [7]spare [8]open [9]avg [10]high [11]games [12]bestRollsJson
        const rank = row[0];
        const bestTs = row[1];
        const name = row[2];
        ...
        tr.onclick = function () {
          if (rollsJson) {
            ...
            alert("詳細データがありません（過去データが旧形式の可能性）");
          }
        };
...
/**********************
 * Stats（直近20）
 * 表記：シュート率 / ストライク率 / スペア率 / オープン率 / 平均入れ球
 * 平均入れ球：1投あたり平均入球（=合計ピン/投球数）
 **********************/
